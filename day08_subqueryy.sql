...
-- 3. ADIM: bu 2 adimi birlestirecegiz
SELECT isim, sirket
FROM calisanlar
...
SELECT sirket, calisanlar_sayisi, (SELECT AVG(maas) 
								   FROM calisanlar
                                   WHERE sirketler.sirket = calisanlar.sirket) AS ort_maas
FROM sirketler;
/*----------------------------------------------------------------
...
SELECT sirket, (SELECT MAX(maas)
				FROM calisanlar
                WHERE sirketler.sirket = calisanlar.sirket) max_maas -- AS yazmayadabiliriz
FROM sirketler;                
...
SELECT sirket, calisanlar_sayisi, (SELECT SUM(maas)
								   FROM calisanlar
                                   WHERE calisanlar.sirket = sirketler.sirket) toplam_maas
FROM sirketler;
​
/* ======================== EXISTS CONDITION ===========================
...
urun_isim'ini ve urunleri nisan ayinda alan musterilerin isimlerini getiren bir Query yaziniz.
----------------------------------------------------------------*/ 
